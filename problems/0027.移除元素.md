        ...
        for (int i = 0; i < size; i++) {
            if (nums[i] == val) { // 发现需要移除的元素，就将数组集体向前移动一位
                for (int j = i + 1; j < size; j++) {
                    nums[j - 1] = nums[j]; // 把nums[i]之后的元素在数组中向前移动
                }
    ...
    int removeElement(vector<int>& nums, int val) {
        ...
        for (int fastIndex = 0; fastIndex < nums.size(); fastIndex++) {
            if (val != nums[fastIndex]) {
                nums[slowIndex++] = nums[fastIndex];  // 只有当当前元素值不等于val的时候，将当前元素值用其他元素覆盖，slowIndex的指针才移动
            }
        }
        return slowIndex;  // 这里返回的是新数组的长度
    }
};
```
    ...
    int removeElement(vector<int>& nums, int val) {
        ...
        while (leftIndex <= rightIndex) {
            ...
            while (leftIndex <= rightIndex && nums[leftIndex] != val){
                ++leftIndex;  // 当nums[leftIndex] == val的时候跳出while循环 
            }
            // 找右边不等于val的元素
            while (leftIndex <= rightIndex && nums[rightIndex] == val) {
                -- rightIndex;  // 当nums[rightIndex] != val的时候跳出while循环 
            }
            // 将右边不等于val的元素覆盖左边等于val的元素
            if (leftIndex < rightIndex) {
                nums[leftIndex++] = nums[rightIndex--];  // 先使用leftIndex对nums进行索引操作，然后再对leftIndex加1
            }
        }
        return leftIndex;   // leftIndex一定指向了最终数组末尾的下一个元素；题目要求只需要考虑长度，不需要考虑元素重复、顺序
    }
};
```
